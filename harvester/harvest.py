"""Orchestrator for games.ofancientrome.org.
...
manifest; the spine and the other enrichers survive), then writes:
  ...
  - data/manifest.json      generated_at, totals, categories, per-source status, launcher
...
_PACK_DROP = {
    "category", "category_label", "latin", "aka", "tagline", "reconstruction",
    "wp", "qid", "players", "duration", "_nodes",
}
# fields promoted into the compact index (data/games.json)
INDEX_FIELDS = [
    "id", "name", "latin", "category", "category_label", "players", "duration",
    "tagline", "reconstruction", "wp", "qid",
]
...
def main():
    ...
    print(f"games.ofancientrome.org harvest — {len(games)} games, "
    ...
    for g in games:
        ...
        row = {k: g.get(k) for k in INDEX_FIELDS}
        row["pigment"] = next((c["pigment"] for c in categories if c["key"] == g["category"]), None)
        row["thumb"] = (g.get("board") or {}).get("image")
        row["has_dice"] = bool(g.get("dice"))
        index_games.append(row)

    generated = datetime.datetime.now(datetime.timezone.utc).isoformat(timespec="seconds")

    index = {
        "generated_at": generated,
        "title": spine.get("title", "Games of Ancient Rome"),
