/* app.js — Games of Ancient Rome catalogue.
 *
...
(function () {
  ...
  const state = { manifest: null, games: [], byId: {}, cat: "all", q: "", detailCache: {} };
  ...
  /* ---------- room codes (Roman flavour) ---------- */
  const WORDS = ["forum", "thermae", "circus", "subura", "palatine", "ostia", "aqua",
    "tabularium", "rostra", "campus", "velabrum", "carcer", "lupanar", "horrea"];
  ...
  /* ---------- boot ---------- */
  (async function init() {
    document.documentElement.dataset.theme = localStorage.getItem("goar-theme") || "light";
    ...
    try {
      ...
      renderStats();
      renderCatFilter(index.categories || manifest.categories || []);
      renderLanes();
  ...
  function toggleTheme() {
    ...
    localStorage.setItem("goar-theme", next);
  }
  ...
  function renderStats() {
    ...
    if (t.with_dice) add(t.with_dice, "with dice");
    if (t.reconstructions) add(t.reconstructions, "reconstructed");
  }

  function renderCatFilter(categories) {
    ...
    const mk = (key, label, pigment) => {
      ...
      b.onclick = () => { state.cat = key; nav.querySelectorAll(".cat-chip").forEach((c) => c.classList.remove("active")); b.classList.add("active"); renderLanes(); };
      return b;
    };
    nav.appendChild(mk("all", "All", null));
    categories.forEach((c) => nav.appendChild(mk(c.key, c.label, c.pigment)));
  }
  ...
  function matches(g) {
    ...
    const hay = [g.name, g.latin, g.tagline, g.category_label, (g.aka || []).join(" ")].join(" ").toLowerCase();
  ...
  function card(g, pigment) {
    ...
    body.appendChild(el("div", "card-cat", g.category_label || g.category));
    body.appendChild(el("h3", null, g.name));
    if (g.latin && g.latin !== g.name) body.appendChild(el("div", "latin", g.latin));
    body.appendChild(el("p", "card-tag", g.tagline || ""));
    const meta = el("div", "card-meta");
    if (g.players) meta.appendChild(el("span", "pill", g.players + " players"));
  ...
  function renderDetail(g) {
    ...
    if (g.latin && g.latin !== g.name) head.appendChild(el("p", "detail-latin", g.latin));
    ...
    if (g.reconstruction) {
      ...
      note.appendChild(document.createTextNode("No ancient source records how this game was played. The rules below are a modern, plausible reconstruction — offered so the board can be enjoyed, not presented as recovered fact."));
    ...
    if (g.dice && g.dice.length) {
      const ds = el("section");
      ds.appendChild(el("h3", "sec-h", "Dice"));
      g.dice.forEach((die) => {
